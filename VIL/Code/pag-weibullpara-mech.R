...
failures<-c(1992,3408,1848,1896,840)
#without suspension
...
plot(survi(c(0:T),eta,beta),pch=14,axes=FALSE,ylim=c(0,1),ylab="", xlab="",col="red",type="l",lwd=2,cex=1, main=paste("Mechanical Seal - bias"))
#  par(new=TRUE)
...
box()
legend("topright",inset=.05,legend=c(paste("Mechanical Seal - bias")), text.col="black",lty=c(1),col=colors[i], horiz=F,cex=0.8,bg = "gray90")
...
failures<-c(1992,3408,1848,1896)
#without suspension
median_percentile_ranks<-getPPP(failures)
print(median_percentile_ranks)#[,2]
MLEfit<-mlefit(mleframe(failures))
...
da1 <- wblr.fit(da1, col="darkgreen")
#da1 <- wblr.conf(da1, col="blue")
plot(da1, main=paste("Mechanical Seal - unbias"))
...
#  par(mar=c(5, 4, 4, 6) + 0.1)
plot(survi(c(0:T),eta,beta),pch=14,axes=FALSE,ylim=c(0,1),ylab="", xlab="",col="red",type="l",lwd=2,cex=1, main=paste("Mechanical Seal - unbias"))
#  par(new=TRUE)
axis(2, ylim=c(0,1),col="darkblue",las=1)  ## las=1 makes horizontal labels
axis(1,pretty(range(c(0:T)),breakpoint))
mtext(expression(paste("Time (hours)")),side=1,col="black",line=2.5)  
mtext(expression(paste("Reliability")),side=2,col="black",line=2.5) 
box()
legend("topright",inset=.05,legend=c(paste("Mechanical Seal - unbias")), text.col="black",lty=c(1),col=colors[i], horiz=F,cex=0.8,bg = "gray90")
