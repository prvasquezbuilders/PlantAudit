...
maxalpha<- 0.1
step=seq(0.001,maxalpha, by =0.01)

alphasa<- matrix(double(1),nrow=length(step),ncol=1)
...
#SA on alpha parameter
  ...
  for (k in 1:maxobj){
    ...
    png(filename=paste("E:/OP18REFCS03/Stations/PAG/Report/figures/ch05_fig_saalpha_pump",k,".png",sep=""),width = 800, height = 500)  
    ...
    plot(alphasa, Cost[k,], pch=2, axes=FALSE, ylim=c(0,plotimpact), xlab="", ylab="", type="b",col="blueviolet", lwd=2, main=c(paste(data$assets[k])))
    ...
    axis(1,pretty(range(alphasa),YearMax))
    box()
    
    par(new=TRUE)
    
    ## Plot the second plot and put axis scale on right
    plot(alphasa, OTW[k,], pch=4,  xlab="", ylab="", ylim=c(0,plottime), 
    ...
    mtext(expression(paste("Parameter ", alpha)),side=1,col="black",line=2.5)  
