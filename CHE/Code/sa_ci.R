...
csa<- matrix(double(1),nrow=length(step),ncol=1)
...
#  plot.new()
  ...
  png(filename=paste("E:/OP18REFCS03/Stations/VIL/Code/figures/ch05_fig_sacipi_pump",i,".png",sep=""),width = 800, height = 500) 
 ...
 #   cat(paste(" Optimal IS for pump #",i,  "\n"))
      ...
      omega[t,i]=(F_cost[t,i]+S[t,i]*csa[u]*exp(-rho*t))/E[t,i]
  ...
  plot(csa, Cost[i,], pch=2, axes=FALSE, ylim=c(0,max(Cost[i,]*2)), xlab="", ylab="", type="b",col="blueviolet", lwd=2)
 ...
 # axis(1,pretty(range(csa),T))
  axis(1,c(seq(0,1, by=0.01)))
  box()
  
  par(new=TRUE)
  
  ## Plot the second plot and put axis scale on right
  plot(csa, OTW[i,], pch=4,  xlab="", ylab="", ylim=c(0,T), 
  ...
  mtext(expression(paste("Ratio PI/CI ")),side=1,col="black",line=2.5)  
