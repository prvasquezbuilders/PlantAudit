...
betasa<- matrix(double(1),nrow=length(step),ncol=1)
...
#  plot.new()
  ...
  png(filename=paste("E:/OP18REFCS03/Stations/VIL/Code/figures/ch05_fig_betasa_pump",i,".png",sep=""),width = 800, height = 500) 
 ...
 #   cat(paste(" Optimal IS for pump #",i,  "\n"))
      ...
      for (t in 1: T){
      F_cost[t,i]=integrate(f_cost,0,t,eta[i],betasa[u],rho)$val
      FF[t,i]<-1-exp(-(1/eta[i]*t)^betasa[u])
      S[t,i]=exp(-(1/eta[i]*t)^betasa[u])
      E[t,i]=integrate(survi,0,t,eta[i],betasa[u])$val
  ...
  plot(betasa, Cost[i,], pch=2, axes=FALSE, ylim=c(0,max(Cost[i,]*2)), xlab="", ylab="", type="b",col="blueviolet", lwd=2)
  axis(2, ylim=c(0,Cost[i,]*2),col="black",las=1)  ## las=1 makes horzontal labels
  mtext(expression(paste("Impacts  (",, "mus)")),side=2,col="black",line=5)
  #axis(1,pretty(range(betasa),3))
  axis(1,c(seq(1,3, by=0.2)))
  box()
  
  par(new=TRUE)
  
  ## Plot the second plot and put axis scale on right
  plot(betasa, OTW[i,], pch=4,  xlab="", ylab="", ylim=c(0,T), 
  ...
  mtext(expression(paste("Parameter ", beta)),side=1,col="black",line=2.5)  
...
betasaresult<-data.frame(OTW, Cost)
print(betasaresult)
print(Cost)
